...
var VaccinePaceChart = /*#__PURE__*/function () {
  function VaccinePaceChart() {
    _classCallCheck(this, VaccinePaceChart);
  ...
  _createClass(VaccinePaceChart, [{
     ...
     */
    value: function draw() {
      var rawData = this.data(); // Data passed to your chart

      var props = this.props(); // Props passed to your chart

      var data = Object.keys(rawData).map(function (isoAlpha2) {
        return client.getCountry(isoAlpha2);
      }).filter(function (c) {
        return c.dataProfile.population && c.dataProfile.population.d > 1000000;
      }).map(function (country) {
        var avgs = rawData[country.isoAlpha2].slice().reverse();
        var max = Math.max.apply(Math, _toConsumableArray(avgs));
        var length = avgs.length;
        return {
          country: country,
          avgs: avgs,
          max: max,
      ...
      var allPoints = flatten__default['default'](data.map(function (d) {
        var country = d.country,
            avgs = d.avgs;
        return avgs.map(function (avg, i) {
          return {
            country: country,
            avg: avg,
            i: i
      ...
      var alphaScale = d3Scale.scaleLinear().domain([0, d3Array.max(data, function (d) {
        return d.last;
      })]).range([0, 0.6]);
      var delaunay = d3Delaunay.Delaunay.from(allPoints, function (d) {
        return xScale(xScale.domain()[1] - d.i);
      }, function (d) {
        return yScale(d.avg);
      });
      ...
      plot.appendSelect('rect').attr('x', 0).attr('y', 0).attr('height', height).attr('width', width).style('fill', 'transparent').style('cursor', 'crosshair').on('mousemove touchmove', function (event) {
        ...
        plot.select("path.country-".concat(country.isoAlpha2)).style('stroke-width', 2).attr('stroke', 'url(#gradient-highlight)');
  ...
  return VaccinePaceChart;
}();

module.exports = VaccinePaceChart;
